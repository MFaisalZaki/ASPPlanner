...
class ASPPlanner:
    def __init__(self, problem, encoder_type):
        ...
        self.base_formula  = self._load_asp_encoding_formula(encoder_type)
        self.logs          = []
    
    def _load_asp_encoding_formula(self, encodingname):
    ...
    def _construct_action(self, action_tuple):
    ...
    def _extract_plan(self, answer):
        self.actions = sorted(map(lambda a: ASPOccursFluent(a), filter(lambda a: 'occurs' in str(a), answer)), key=lambda a:a.timestep)
        _plan = SequentialPlan(list(map(self._construct_action, map(lambda e: (e['action'], *[eval(a)['value'] for a in e['arguments']]), map(lambda a: self.plan_parser.parse_plan_fact(a.fact_str), self.actions)))))
        ...
        for n in range(0, 1000):
            ...
            with ctl.solve(yield_=True) as solution_iterator:
                ...
                for solution in solution_iterator:
                    _plan = self._extract_plan(set(solution.symbols(shown=True)))
                    if len(_plan.actions) > 0: break
