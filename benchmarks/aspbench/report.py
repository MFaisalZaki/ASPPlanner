"""Paper-ready tables and plots: ``aspbench report``.
...
Plots need matplotlib (``pip install -e "benchmarks[plots]"``); without it the
...
def report(args) -> int:
    ...
    if figures is None:
        print('Plots   : skipped -- matplotlib not installed '
              '(pip install -e "benchmarks[plots]")')
    else:
...
def _latex_table(caption: str, label: str, column_spec: str,
    ...
    lines = [
        '% Generated by `aspbench report`. Requires \\usepackage{booktabs}.',
