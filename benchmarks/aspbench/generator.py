...
The unit of work is a single (planner, task) pair, run by ``aspbench solve``.
...
def generate(args) -> int:
    ...
    with open(sandbox.tasks_file, 'w') as handle:
        json.dump({'summary': summarize(tasks), 'tasks': [t.to_dict() for t in tasks]},
                  handle, indent=2)
    ...
    for planner in experiment.planners:
        ...
        for task in tasks:
            ...
            if args.skip_existing and os.path.exists(sandbox.result_file(planner.tag, task.slug)):
                skipped_done += 1
                continue
            commands.append(prefix + solve_command(planner, task, sandbox, time_limit, memory_limit))
...
def solve_command(planner: PlannerConfig, task: Task, sandbox: Sandbox,
                  time_limit: int, memory_limit: int) -> str:
    """The ``aspbench solve`` invocation for one (planner, task) pair.
...
def _launcher_prefix(args) -> str:
    """What has to happen before ``aspbench`` is on PATH on the compute node."""
...
def _write_slurm_arrays(sandbox: Sandbox, experiment: Experiment,
    ...
    for tag, commands in per_planner.items():
        ...
        for number, (start, end) in enumerate(chunks, start=1):
            ...
            job_name = f'aspbench-{tag}{suffix}'
...
def _write_local_runner(sandbox: Sandbox, per_planner: Dict[str, List[str]], jobs: int) -> None:
    """A no-slurm fallback: the same commands through ``xargs -P``."""
...
def _report(sandbox: Sandbox, experiment: Experiment, tasks: Sequence[Task],
    ...
    for tag, commands in per_planner.items():
        print(f'  {tag:<24} {len(commands):>6} commands')
    if skipped_done:
