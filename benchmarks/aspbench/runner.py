...
This is the only stage that imports ``unified_planning`` and ``aspplanners``,
and the only one that runs on a compute node.
...
``TIMEOUT``/``MEMOUT`` row with the horizon it reached, not as a missing file
and a line in the slurm accounting log. The scheduler's limits stay in place as
the backstop for the cases the process cannot catch itself -- notably a solve
that dies inside clingo's grounder, where no Python handler gets to run.
"""
...
EXHAUSTED = 'EXHAUSTED'        # search bound (max_horizon) reached, task may still be solvable
TIMEOUT = 'TIMEOUT'
MEMOUT = 'MEMOUT'
...
def solve(args) -> int:
    """Entry point for ``aspbench solve``. Always writes a result file."""
    ...
    try:
        ...
        factory = up_shortcuts.get_environment().factory
        engine_class = factory.engine(engine)
        if hasattr(engine_class, 'supports') and not engine_class.supports(problem.kind):
        ...
        with up_shortcuts.OneshotPlanner(name=engine, params=params) as planner:
            outcome = planner.solve(problem, timeout=remaining)
...
def _enter_workdir(run_dir: Optional[str], tag: str, slug: str) -> Optional[str]:
    ...
    directory, and it is not the only tool that does. On a cluster the working
    directory is wherever ``sbatch`` was called from -- shared by every task in
    the array -- so two concurrent runs would read and overwrite each other's
    intermediate files. The directory name carries the planner tag, the task and
    the pid, so it is unique per run even when the same pair is re-run or two
    planners hit the same instance at the same moment.
    """
    if not run_dir:
        return None
    try:
...
def _install_limits(time_limit: Optional[int], memory_limit: Optional[int]) -> None:
    ...
    The alarm covers the search (the planner's clingo solve runs async, so a
    Python handler does get scheduled), and the address-space limit turns
    runaway grounding into a ``MemoryError`` we can report. Signals the
    scheduler sends on the way to a kill are caught too, so a job that is about
    to be cancelled still leaves a result behind.
    """
...
def _resolve_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Only ``compilationlist`` needs it: its second element names a
    ``CompilationKind`` member, which JSON cannot hold.
    """
    from unified_planning.shortcuts import CompilationKind

    resolved = dict(params)
    for key in ('compilationlist', 'compilation-list', 'compliation-list'):
        entries = resolved.pop(key, None)
        if entries is None:
            continue
        pipeline = []
...
def _validate(problem, plan, result: Dict[str, Any]) -> Optional[bool]:
    """Re-check the plan against the *original* problem.

    The planners validate internally before returning, so this is a
    belt-and-braces check on the harness side: it is the number the coverage
    table is allowed to trust.
    """
    try:
        from aspplanners.common.validation import validate_plan
        is_valid, reason = validate_plan(problem, plan)
...
def _dump_traceback(args, tag: str, slug: str, error: Exception) -> None:
    ...
    try:
        ...
        with open(os.path.join(errors_dir, f'{tag}__{slug}.log'), 'w') as handle:
